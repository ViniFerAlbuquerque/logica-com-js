function jogar() {
    ...
    if (!jogadorHumano) {
        alert('[[ERRO!] ► Escolha sua jogada!]')
        return 
    } 
    const jogadas = ['Pedra', 'Papel', 'Tesoura'] 
    const jogadorIA = jogadas[Math.floor(Math.random() * jogadas.length)] 
    const escolhaHumano = jogadorHumano.value // definir value="Pedra", etc., no HTML
...
const resposta = document.querySelector('article.resposta') 
